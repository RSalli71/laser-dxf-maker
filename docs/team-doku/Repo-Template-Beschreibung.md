...
```
PROJECT_BRIEF.md ausfüllen → Requirements in docs/requirements/REQUIREMENTS.md erstellen → Architektur in docs/ARCHITECTURE.md festlegen → Developer-Phase in src/ → QA, Review und Gatekeeper
```

Die verbindliche Reihenfolge steht in `docs/workflow.md`.
...
Das Template enthält **8 spezialisierte Agent-Personas** unter `.claude/agents/`. Diese dienen als Kontext für Claude Code und ermöglichen rollenbasiertes AI-Development.

| Agent | Datei | Aufgabe |
|---|---|---|
| **Requirements Engineer** | `requirements-engineer.md` | Analysiert PROJECT_BRIEF.md und erstellt die Anforderungen in `docs/requirements/REQUIREMENTS.md` |
...
| **Backend Developer** | `backend-developer.md` | Implementiert API-Routen, Server-Logik, Import-Pipelines und Library-Module |
...
2. Erstellt strukturierte Anforderungen in `docs/requirements/REQUIREMENTS.md`
3. Übergibt damit an die nächsten Workflow-Schritte aus `docs/workflow.md`

**Aufruf:** Nach dem Ausfuellen von `PROJECT_BRIEF.md` zuerst `docs/workflow.md` gegenlesen und dann `.claude/agents/requirements-engineer.md` als Einstieg verwenden.
...
| **Linting** | ESLint + eslint-config-next | ^9.19 / ^16.1.6 |
...
| `lint:fix` | `eslint . --fix` | ESLint mit Auto-Fix |
...
1. **Orientierung und Workflow** — kanonische `docs/`-Dateien, Rollen und Skill-Quelle
2. **Kleine, sichere Schritte** — Max. 1–3 Dateien pro Änderung, Plan vorher benennen
3. **Qualität** — Einfache Lösungen bevorzugen, keine Breaking Changes ohne Hinweis
4. **Sicherheit** — Niemals Secrets in Code, Logs oder Doku
5. **Doku-Pflicht** — DEVLOG bei relevanten Änderungen, DECISIONS bei "Warum?"-Fragen
6. **Next.js Konventionen** — App Router, Server/Client Components, Metadata
7. **Styling & UI** — Tailwind v4, shadcn/ui CLI, `cn()` Helper, Komponenten-Struktur
8. **Animationen und Handoffs** — Framer Motion-Regeln plus Uebergaben entlang `docs/workflow.md`
...
| **next-best-practices** | Vercel Labs | 20+ Dateien | Next.js App Router Patterns: File Conventions, RSC Boundaries, Async Patterns, Data Fetching, Error Handling, Metadata, Image/Font Optimierung |
...
### Skill-Struktur und Tool-Kompatibilität

Die Skills liegen kanonisch unter `.agents/skills/`. Darauf zeigen kompatible Pfade für verschiedene Tools.

- **Kanonische Quelle**: `.agents/skills/`
- **Claude Code**: `.claude/skills/` als Kompatibilitätspfad
- **GitHub Copilot und generische CLI-Tools**: `skills/` als neutraler Kompatibilitätspfad
- **Optional**: `.vibe/skills/` als Kompatibilitätspfad, wenn Mistral Vibe lokal genutzt wird

Auf Windows werden diese Kompatibilitätspfade als Junction-Points angelegt. Manche Dateibaum-Tools zeigen sie daher nicht wie normale Ordner an, Zugriffe funktionieren aber normal.
...
| `docs/workflow.md` | Reihenfolge der Phasen, Agent-Zuordnung und Output-Pfade | Änderungen am Delivery-Workflow |
| `docs/requirements/REQUIREMENTS.md` | Kanonische Anforderungen und Akzeptanzkriterien | Nach Briefing oder Scope-Änderungen |
| `docs/reports/TEST_REPORT.md` | QA-Ergebnisse und Testabdeckung | Nach QA- oder Test-Läufen |
| `docs/knowledge/` | Obsidian-Wissensnotizen und Analyse-Templates | Wenn Wissensbasis oder Analyse-Layer erweitert werden |
| `docs/team-doku/` | Team-interne Arbeitsanweisungen wie Debug- und Ship-Safe-Checklisten | Bei Änderungen an Teamprozessen |
...
# Danach: docs/workflow.md gegenlesen
# Einstieg ueber .claude/agents/requirements-engineer.md
```
...
# Erst docs/workflow.md folgen, dann Code in src/ schreiben
# Spikes und Experimente zuerst in vibe/
# Daten/Samples in data/, Hilfsprogramme in tools/, Tests in tests/
...
# Anforderungen in docs/requirements/REQUIREMENTS.md
# Test-Report in docs/reports/TEST_REPORT.md
```
